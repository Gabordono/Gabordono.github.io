export const creditFraudCode = String.raw`"""
=============================================================================
...
def train_models(X_train, y_train):
    ...
    print("    [✓] Logistic Regression kész")
...
def plot_dashboard(results: dict, models: dict, X_test, y_test) -> None:
    ...
    # ── Panel 3: Confusion Matrix (legjobb modell) ───────────────────────────
...
def print_summary(results: dict) -> None:
    ...
    print(f"""
  Gépi tanulásalapú csalásdetektáló rendszer Pythonban –
...
export const creditFraudSnippet = String.raw`# Fraud Detection ML – Kulcsrészlet
# Dataset: Kaggle Credit Card Fraud (284,807 tranzakció, 0.17% fraud)
...
# SMOTE – osztályegyensúly helyreállítása (csak train seten!)
smote = SMOTE(random_state=42)
X_train_res, y_train_res = smote.fit_resample(X_train, y_train)
# Eredmény: {0: 227451, 1: 227451}  ← kiegyensúlyozott

# Random Forest – imbalanced adatokra hangolva
rf = RandomForestClassifier(
...
print(f"AUC-ROC:  {roc_auc_score(y_test, y_prob):.4f}")   # → 0.9743
print(f"F1 Score: {f1_score(y_test, y_pred):.4f}")         # → 0.8812
# Elkapott fraud: 82 / 98  (Recall: 83.67%)
`;
